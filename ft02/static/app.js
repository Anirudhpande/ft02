...
async function loadSectors() {
    try {
        ...
        data.sectors.forEach(s => {
            ...
            opt.value = s.name;
            opt.textContent = `${s.name} (risk: ${s.risk_score}/10)`;
...
async function handleSubmit(e) {
    ...
    const payload = {
        gstin: gstin,
        business_name: document.getElementById('name-input').value.trim() || null,
        sector: document.getElementById('sector-select').value || null,
        business_age: parseInt(document.getElementById('age-input').value) || null,
        constitution: document.getElementById('constitution-select').value || null,
        simulate_fraud: document.getElementById('fraud-check').checked,
    };
